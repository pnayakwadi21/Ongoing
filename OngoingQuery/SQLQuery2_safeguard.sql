...
--, COALESCE(cteTypeOfInjury.[Type of Injury], 'No Injury') AS [Type of injury caused]
--, CASE	WHEN hospReq.udv_string = 'N' THEN 'No'
--		WHEN hospReq.udv_string = 'Y' THEN 'Yes'
--		ELSE '' END AS [Hospital Treatment]
--, 'RIDDOR Reporting' AS [RIDDOR Reporting]
--, COALESCE(ageAtTime.udv_string, '') AS [RIDDOR Age of person affected at time]

--, COALESCE(personGenderDesc.cod_descr, '') AS [RIDDOR Gender of person affected Description]
...
--, COALESCE(locDetails.UDV_TEXT, '') AS [Location Description]
--, 'Incident details' AS [Incident details]
...
--LEFT OUTER JOIN dbo.udf_values initInspec  WITH (NOLOCK)
--ON initInspec.mod_id = 3 AND initInspec.field_id = 69 AND initInspec.cas_id = incidents_main.recordid
--LEFT OUTER JOIN dbo.udf_values suppProv  WITH (NOLOCK)
--ON suppProv.mod_id = 3 AND suppProv.field_id = 70 AND suppProv.cas_id = incidents_main.recordid
--LEFT OUTER JOIN dbo.udf_values furtherActReq  WITH (NOLOCK)
--ON furtherActReq.mod_id = 3 AND furtherActReq.field_id = 78 AND furtherActReq.cas_id = incidents_main.recordid
--LEFT OUTER JOIN dbo.udf_values furtherAction  WITH (NOLOCK)
--ON furtherAction.mod_id = 3 AND furtherAction.field_id = 79 AND furtherAction.cas_id = incidents_main.recordid
